...
export default function ExpensesPage() {
  ...
  async function handleScanBill(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      // Dynamically import Tesseract.js (runs in browser only)
      ...
      // Send extracted text to API for Groq parsing
      ...
      setForm({
        ...
        covered_amount: "0",
        expense_date:
          data.extracted.expense_date ?? new Date().toISOString().split("T")[0],
        member_id: "",
        category_id:
          categories.find(
            (c) =>
              c.name.toLowerCase() ===
              (data.extracted.category ?? "").toLowerCase(),
          )?.id ?? "",
      ...
      alert(
        err.message ??
          "Failed to scan bill. Please try again with a clearer photo.",
      );
    } finally {
      setScanning(false);
  ...
  const totalOOP = filtered.reduce(
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Track all medical spending across your family
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-2xl border border-border p-4">
          ...
          <div className="text-xl font-semibold text-emerald-600">
            {formatCurrency(totalFiltered - totalOOP)}
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-border p-16 text-center">
          ...
          <p className="text-muted-foreground text-sm mb-6">
            {expenses.length === 0
              ? "Add your first medical expense to get started."
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <span className="text-sm">
                        {(expense.family_members as any)?.name ?? (
                    ...
                    <td className="px-4 py-4 text-right text-sm text-red-600 font-medium">
                      {formatCurrency(
                        Number(expense.amount) - Number(expense.covered_amount),
                      )}
                    </td>
                    <td className="px-4 py-4">
                      <button
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, covered_amount: e.target.value })
                    }
                    placeholder="0.00"
                    className="input-field"
                  />
