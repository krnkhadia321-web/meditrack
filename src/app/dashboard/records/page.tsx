...
export default function RecordsPage() {
  ...
  async function handleScanRecord(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch("/api/ocr", {
        ...
        body: JSON.stringify({ text, type: "record" }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      setForm({
        member_id: "",
        record_type: data.extracted.record_type ?? "other",
        title: data.extracted.title ?? "",
        notes: data.extracted.notes ?? "",
        record_date:
          data.extracted.record_date ?? new Date().toISOString().split("T")[0],
      });
      setShowScanReview(true);
      setShowModal(true);
    } catch (err: any) {
      alert(err.message ?? "Failed to scan document.");
    } finally {
      setScanning(false);
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`badge text-xs ${TYPE_COLORS[r.record_type] ?? "bg-gray-100 text-gray-600"}`}
                  >
                    {r.record_type.charAt(0).toUpperCase() +
                      r.record_type.slice(1)}
                  </span>
                </div>
                <button
              ...
              <p className="text-xs text-muted-foreground mb-3">
                {(r.family_members as any)?.name ?? "Unknown"} ·{" "}
              ...
              <div>
                ...
                {members.length === 0 && (
                  <p className="text-xs text-amber-600 mt-1">
                    ⚠️ Add a family member first before adding records.
                  </p>
                )}
              </div>
              <div>
