...
import {
  Activity, Plus, X, Loader2, Trash2, AlertCircle, CheckCircle2, TrendingUp, TrendingDown, Minus
} from 'lucide-react'
import {
...
function getStatus(type: string, value: number, value2?: number | null): 'normal' | 'warning' | 'critical' {
  const cfg = VITAL_CONFIGS[type]
  if (!cfg) return 'normal'
  const outOfRange = value < cfg.normalMin || value > cfg.normalMax
  const outOfRange2 = value2 !== null && value2 !== undefined && cfg.normalMin2 !== undefined
    ? value2 < cfg.normalMin2! || value2 > cfg.normalMax2!
    : false
  if (!outOfRange && !outOfRange2) return 'normal'
  const severelyOut = value < cfg.normalMin * 0.85 || value > cfg.normalMax * 1.15
  return severelyOut ? 'critical' : 'warning'
}

function formatValue(v: Vital): string {
  if (v.type === 'blood_pressure' && v.value2) return `${v.value}/${v.value2} ${v.unit}`
...
export default function VitalsPage() {
  ...
  const latestByType = Object.keys(VITAL_CONFIGS).reduce((acc, type) => {
    const latest = memberVitals.filter(v => v.type === type)
    acc[type] = latest
    return acc
  }, {} as Record<string, Vital[]>)

  const alerts = Object.entries(latestByType)
    .filter(([type, v]) => v.length > 0 && getStatus(type, Number(v[0].value), v[0].value2) !== 'normal')
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Track health metrics and spot trends early</p>
        </div>
      ...
      {alerts.length > 0 && (
        <div className="space-y-2 mb-6">
          {alerts.map(([type, vList]) => {
            const status = getStatus(type, Number(vList[0].value), vList[0].value2)
            const cfg = VITAL_CONFIGS[type]
            return (
              <div key={type}
                ...
                <AlertCircle className={`w-5 h-5 shrink-0 ${status === 'critical' ? 'text-red-500' : 'text-amber-500'}`} />
                <p className={`text-sm ${status === 'critical' ? 'text-red-800' : 'text-amber-800'}`}>
                  <strong>{cfg.label}</strong> reading of{' '}
                  <strong>{formatValue(vList[0])}</strong> is outside the normal range.{' '}
                  {status === 'critical' ? 'Please consult a doctor.' : 'Monitor closely.'}
                </p>
              </div>
            )
          })}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {Object.entries(VITAL_CONFIGS).map(([type, cfg]) => {
              ...
              const status = latest ? getStatus(type, Number(latest.value), latest.value2) : 'normal'
              ...
              return (
                <button key={type}
                  ...
                  <div className="text-xl mb-2" style={{ fontSize: '20px' }}>{cfg.icon}</div>
                  <div className="text-xs font-medium text-muted-foreground mb-1">{cfg.label}</div>
                  {latest ? (
                    <>
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        {trend === 'up' && <TrendingUp className="w-3 h-3 text-muted-foreground" />}
                        {trend === 'down' && <TrendingDown className="w-3 h-3 text-muted-foreground" />}
                        {trend === 'stable' && <Minus className="w-3 h-3 text-muted-foreground" />}
                        <span className="text-xs text-muted-foreground">{typeVitals.length} readings</span>
                      </div>
                    </>
                  ) : (
          ...
          {chartData.length > 0 ? (
            <div className="bg-white rounded-2xl border border-border p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-xs text-muted-foreground">{chartData.length} readings</span>
              </div>
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={chartData}>
                  ...
                  <Tooltip
                    ...
                    formatter={(val: number) => [`${val} ${activeCfg.unit}`, activeCfg.label]}
                  />
                  {activeCfg.normalMin > 0 && (
                    <ReferenceLine y={activeCfg.normalMin} stroke="#10b981" strokeDasharray="4 4" strokeWidth={1} />
                  )}
                  {activeCfg.normalMax < 999 && (
                    <ReferenceLine y={activeCfg.normalMax} stroke="#ef4444" strokeDasharray="4 4" strokeWidth={1} />
                  )}
                  <Line
                    type="monotone" dataKey="value"
                    stroke={activeCfg.color} strokeWidth={2}
              ...
              <div className="flex items-center gap-4 mt-3 text-xs text-muted-foreground">
                <span className="flex items-center gap-1">
                  <span className="inline-block w-6 h-0.5 bg-emerald-500" style={{ borderTop: '1px dashed #10b981' }}></span>
                  Min normal
                </span>
                <span className="flex items-center gap-1">
                  <span className="inline-block w-6 h-0.5 bg-red-400" style={{ borderTop: '1px dashed #ef4444' }}></span>
                  Max normal
                </span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-border p-10 text-center">
              <div className="text-3xl mb-3" style={{ fontSize: '32px' }}>{activeCfg.icon}</div>
              <h3 className="font-semibold mb-1">No {activeCfg.label} readings yet</h3>
              <p className="text-sm text-muted-foreground mb-4">{activeCfg.description}</p>
              <button onClick={() => {
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      ...
                      <th className="text-left text-xs font-medium text-muted-foreground px-4 py-3">Reading</th>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-sm font-medium mb-1.5 block">Vital type *</label>
                <select value={form.type}
                  onChange={e => setForm({ ...form, type: e.target.value, value: '', value2: '' })}
              ...
              <div className={`grid gap-3 ${VITAL_CONFIGS[form.type]?.placeholder2 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                <div>
                ...
                {VITAL_CONFIGS[form.type]?.placeholder2 && (
                  <div>
                    <label className="text-sm font-medium mb-1.5 block">Diastolic *</label>
                    <input type="number" step="0.1" value={form.value2}
              ...
              <div>
                ...
                <input value={form.notes}
                  ...
                  placeholder="e.g. After exercise, fasting" className="input-field" />
              </div>
